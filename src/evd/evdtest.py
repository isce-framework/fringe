...
def report(cmat):
    '''
    Report analysis of covariance matrix.
    '''
    ...
    w1,v1  = np.linalg.eigh(np.abs(cmat))
    msk1 = (w1<1.0e-6)
    print('Eigen value properties of CORR')
    print('Range: ', np.min(w1), np.max(w1))
    print('Number used: {0} out of {1}'.format(nsar-np.sum(msk1),nsar))
    ...
    print('Top Hadamard')
    print(A[:5,:5])
    print('Bottom Hadamard')
    print(A[-5:,-5:])

    w2,v2  = np.linalg.eigh(A)
    msk2 = (w2<1.0e-6)
    print('Eigen value properties of Final')
    print('Range: ', np.min(w2), np.max(w2))
    print('Number used: {0} out of {1}'.format(nsar-np.sum(msk2),nsar))
